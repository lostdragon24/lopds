...
void insert_book_to_db(DatabaseHandle *db_handle, const char *filepath,
  ...
  log_message(config, "DEBUG", "[INSERT_BOOK_TO_DB] Inserting book: %s",
              filepath);
  log_message(config, "DEBUG", "[INSERT_BOOK_TO_DB] Database type: %d",
              db_handle->db_type);
  ...
  case DB_SQLITE: {
    ...
    if (book_exists(db_handle, filepath, archive_path, internal_path, NULL,
                    config)) {
      log_message(config, "DEBUG",
                  "[INSERT_BOOK_TO_DB] Book already exists, skipping: %s",
                  filepath);
      return;
    }
    ...
    const char *sql = "INSERT INTO books ("
                      ...
                      "genre, series, series_number, year, "
                      "language, publisher, description, last_modified"
                      ") VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, "
                      "CURRENT_TIMESTAMP)";
    ...
    if (rc != SQLITE_OK) {
      log_message(config, "ERROR", "Failed to prepare SQL statement: %s",
                  sqlite3_errmsg(db));
      return;
    }
    ...
    if (rc != SQLITE_DONE) {
      log_message(config, "ERROR", "Failed to insert book: %s (error code: %d)",
