...
int process_inpx_if_enabled(DatabaseHandle *db_handle, Config *config) {
  ...
  log_message(config, "INFO",
              "INPX scanner enabled, looking for INPX files...");

  char *inpx_file = find_inpx_file(config->scanner.books_dir);
  if (!inpx_file) {
    log_message(
        config, "INFO",
        "No INPX file found in books directory, proceeding with regular scan");
    return -1;
  }
  ...
  if (config->scanner.clear_database_inpx) {
    ...
    if (!clear_database(db_handle, config)) {
      log_message(config, "ERROR",
                  "Failed to clear database, aborting INPX import");
      free(inpx_file);
      return -1;
    }
  }
