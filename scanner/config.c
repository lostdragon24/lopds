...
// Вспомогательная функция — безопасный strdup
...
static const char* get_xdg_config_home(void) {
    ...
    if (home) {
        ...
        snprintf(xdg_path, sizeof(xdg_path), "%s/.config", home);
        return xdg_path;
    }
    return NULL;
#endif
...
static char* get_executable_dir(void) {
    ...
    if (GetModuleFileNameA(NULL, exec_dir, sizeof(exec_dir))) {
        ...
        if (!last_sep) last_sep = strrchr(exec_dir, '/');
        if (last_sep) {
            *last_sep = '\0';
            return exec_dir;
        }
...
char* find_config_file(void) {
    ...
    snprintf(possible_paths[count++], PATH_MAX, "./config.ini");
    snprintf(possible_paths[count++], PATH_MAX, "./config/config.ini");

    // 2. XDG (Linux/macOS)
    const char* xdg = get_xdg_config_home();
    if (xdg) {
        snprintf(possible_paths[count++], PATH_MAX, "%s/book_scanner/config.ini", xdg);
    }

    // 3. ~/.config fallback
    const char* home = get_home_dir();
    if (home) {
        snprintf(possible_paths[count++], PATH_MAX, "%s/.config/book_scanner/config.ini", home);
        snprintf(possible_paths[count++], PATH_MAX, "%s/.book_scanner/config.ini", home);
    }

    // 4. Системный путь (только Unix)
#ifndef _WIN32
    snprintf(possible_paths[count++], PATH_MAX, "/etc/book_scanner/config.ini");
    snprintf(possible_paths[count++], PATH_MAX, "/etc/config.ini");
#endif

    // 5. Директория исполняемого файла
    char* exec_dir = get_executable_dir();
    if (exec_dir) {
        snprintf(possible_paths[count++], PATH_MAX, "%s/config.ini", exec_dir);
        snprintf(possible_paths[count++], PATH_MAX, "%s/config/config.ini", exec_dir);
    }

    // Ищем первый существующий файл
    for (int i = 0; i < count && i < 16; i++) {
        if (access(possible_paths[i], R_OK) == 0) {
            // Используем fprintf для stderr, так как config еще не создан
...
Config* read_config(const char *config_path) {
    ...
    if (strlen(config_path) >= 3 &&
        ...
        snprintf(actual_config_path, sizeof(actual_config_path), "%s", config_path);
    } else
#else
    if (config_path[0] == '/') {
        snprintf(actual_config_path, sizeof(actual_config_path), "%s", config_path);
...
#endif
    {
        ...
        char* exec_dir = get_executable_dir();
        if (exec_dir) {
            snprintf(actual_config_path, sizeof(actual_config_path),
        ...
        *equals = '\0';
        ...
        if (strcmp(current_section, "database") == 0) {
            ...
            if (strcmp(key, "books_dir") == 0) {
                ...
                config->scanner.rescan_unchanged = (strcasecmp(value, "yes") == 0 ||
                                                   strcasecmp(value, "true") == 0 ||
                                                   strcmp(value, "1") == 0);
            } else if (strcmp(key, "enable_inpx") == 0) {
                config->scanner.enable_inpx = (strcasecmp(value, "yes") == 0 ||
                                              strcasecmp(value, "true") == 0 ||
                                              strcmp(value, "1") == 0);
            } else if (strcmp(key, "clear_database_inpx") == 0) {
                config->scanner.clear_database_inpx = (strcasecmp(value, "yes") == 0 ||
                                                      strcasecmp(value, "true") == 0 ||
                                                      strcmp(value, "1") == 0);
