    ...
    Config* config)
{
    ...
    while (archive_read_next_header(a, &entry) == ARCHIVE_OK) {
        ...
        if (strstr(filename, "structure.info") || strstr(filename, "collection.info") || strstr(filename, "version.info")) {
        ...
        while (line && *line) {
            ...
            *line_end = '\0';
            ...
            if (strlen(line) > 10) {
                ...
                if (meta.title && strlen(meta.title) > 0 && meta.author && strlen(meta.author) > 0 && file_name && strlen(file_name) > 0) {
                    ...
                    insert_book_to_db(db_handle, file_path, &meta, archive_path, internal_path, meta.file_hash, config);
                    books_imported++;
                    books_in_file++;
