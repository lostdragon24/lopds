...
int import_inpx_collection(const char *inpx_filename, DatabaseHandle *db_handle, Config *config) {
    ...
    if (access(inpx_filename, R_OK) != 0) {
        log_message(config, "ERROR", "Cannot access INPX file: %s (errno: %d)", inpx_filename, errno);
        return 0;
    }

    struct stat st;
    if (stat(inpx_filename, &st) == 0) {
        log_message(config, "DEBUG", "INPX file size: %lld bytes", (long long)st.st_size);
        if (st.st_size == 0) {
            log_message(config, "ERROR", "INPX file is empty: %s", inpx_filename);
            return 0;
        }
    } else {
        log_message(config, "ERROR", "Cannot stat INPX file: %s (errno: %d)", inpx_filename, errno);
        return 0;
    }
    ...
    if (r != ARCHIVE_OK) {
        log_message(config, "ERROR", "Cannot open INPX file as archive: %s", archive_error_string(a));
        archive_read_free(a);
        return 0;
    }
    ...
    while (archive_read_next_header(a, &entry) == ARCHIVE_OK) {
        ...
        if (size > 100 * 1024 * 1024) {
            log_message(config, "DEBUG", "INP file too large: %s (%lld bytes)", filename, size);
            archive_read_data_skip(a);
            continue;
        }
        ...
        if (!content) {
            log_message(config, "ERROR", "Cannot allocate %lld bytes for INP file: %s", size, filename);
            archive_read_data_skip(a);
            continue;
        }
        ...
        if (bytes_read != size) {
            log_message(config, "ERROR", "Failed to read INP file: %s (read %zd of %lld bytes)",
        ...
        log_message(config, "DEBUG", "Processed %d lines in INP file, imported %d books", line_num, books_in_file);
        free(content);
    }

    log_message(config, "DEBUG", "Total archive entries processed: %d", total_entries);
