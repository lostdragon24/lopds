...
class EpubCoverParser extends BaseCoverParser
{
    public function hasCover($book): bool
    {
        ...
        $hasCover = false !== $coverData && strlen($coverData) > 1000;
    ...
    private function findCoverInOpf($opfXml, $baseDir, $relativePath)
    {
        ...
        if (!empty($namespaces)) {
            ...
            foreach ($dc as $meta) {
                if ('meta' == $meta->getName() && isset($meta['name']) && 'cover' == $meta['name']) {
        ...
        foreach ($opfXml->manifest->item as $item) {
            if (isset($item['properties']) && 'cover-image' == (string) $item['properties']) {
        ...
        foreach ($opfXml->manifest->item as $item) {
            ...
            if (false !== stripos($id, 'cover') || false !== stripos($href, 'cover')) {
                if (0 === strpos($mediaType, 'image/')) {
                    return $this->getImageByItem($item, $baseDir, $relativePath);
                }
            }
        }
