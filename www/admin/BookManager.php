...
class BookManager
{
    ...
    public function getBooks($page = 1, $perPage = 20, $filters = [])
    {
        ...
        $orderDir = 'ASC' === strtoupper($orderDir) ? 'ASC' : 'DESC';
    ...
    public function saveBook($data, $file = null)
    {
        ...
        if (!$file || !isset($file['tmp_name']) || UPLOAD_ERR_OK !== $file['error']) {
     ...
     */
    private function extractMetadataFromFile($filePath, $extension)
    {
        $metadata = [
            'title' => '',
            'author' => '',
        ...
        try {
            switch ($extension) {
                case 'fb2':
                    ...
                    if ($content) {
                        // Извлекаем название
                        if (preg_match('/<book-title>(.*?)<\/book-title>/is', $content, $matches)) {
                            $metadata['title'] = trim(strip_tags($matches[1]));
                        }
                        // Извлекаем автора
                        if (preg_match('/<author>.*?<first-name>(.*?)<\/first-name>.*?<last-name>(.*?)<\/last-name>.*?<\/author>/is', $content, $matches)) {
                            $metadata['author'] = trim($matches[2].' '.$matches[1]);
                        }
                        // Извлекаем жанр
                        if (preg_match('/<genre>(.*?)<\/genre>/is', $content, $matches)) {
                            $metadata['genre'] = trim($matches[1]);
                        }
                        // Извлекаем описание
                ...
                case 'epub':
                    // Для EPUB можно использовать простой парсинг container.xml и opf файла
                    $zip = new ZipArchive();
                    if (true === $zip->open($filePath)) {
                        // Читаем container.xml
                        $container = $zip->getFromName('META-INF/container.xml');
                        if ($container && preg_match('/full-path="([^"]+)"/', $container, $matches)) {
                            $opfFile = $matches[1];
                            $opfContent = $zip->getFromName($opfFile);
                            if ($opfContent) {
                                // Извлекаем метаданные из OPF
                                if (preg_match('/<dc:title[^>]*>(.*?)<\/dc:title>/is', $opfContent, $matches)) {
                                    $metadata['title'] = trim(strip_tags($matches[1]));
                                }
                                if (preg_match('/<dc:creator[^>]*>(.*?)<\/dc:creator>/is', $opfContent, $matches)) {
                                    $metadata['author'] = trim(strip_tags($matches[1]));
                                }
                ...
                case 'pdf':
                    // Для PDF используем простое извлечение через exec (если есть pdftotext)
                    if (function_exists('exec')) {
                        $tempFile = tempnam(sys_get_temp_dir(), 'pdf_');
                        exec('pdftotext '.escapeshellarg($filePath).' '.escapeshellarg($tempFile).' 2>/dev/null');
                        if (file_exists($tempFile) && filesize($tempFile) > 0) {
                            $content = file_get_contents($tempFile);
                            $lines = explode("\n", $content, 5);
                            if (!empty($lines[0])) {
                                $metadata['title'] = trim($lines[0]);
                            }
                            if (isset($lines[1])) {
                                $metadata['author'] = trim($lines[1]);
                            }
                            unlink($tempFile);
                        }
                    }
    ...
    public function getStats()
    {
        try {
            ...
            // С рейтингами
            $stmt = $this->db->getConnection()->query('SELECT COUNT(DISTINCT book_id) FROM book_ratings');
            $stats['with_ratings'] = $stmt->fetchColumn();

            // В архивах
            $stmt = $this->db->getConnection()->query('SELECT COUNT(*) FROM books WHERE archive_path IS NOT NULL');
            $stats['in_archives'] = $stmt->fetchColumn();
